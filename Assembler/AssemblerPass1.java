...
class AssemblerPass1 {
    public static void main(String[] args) throws NullPointerException, FileNotFoundException {
        ...
        try {
            ...
            while ((line = br.readLine()) != null && !end) {
                ...
                for (int k = 0; k < tokens.length; k++) {
                    ...
                    if (pos == -1) {
                        ...
                        if (pos != -1) {
                            bw.write("\t(IS," + (pos) + ")");
                            optab_table[optab_cnt++] = new Obj(tokens[k], pos);
                        } else {
                            ...
                            if (pos != -1) {
                                ...
                                pos = search(tokens[k], symb_table, total_symb);
                                if (pos == -1) {
                                    symb_table[total_symb++] = new Obj(tokens[k].substring(0, tokens[k].length() - 1),
                                            loc - 1);
                    ...
                    if (pos == -1) {
                        ...
                        if (pos != -1)
                            ...
                            if (tokens[k].matches("='\\d+'")) {
                                ...
                                bw.write("\t(C," + tokens[k] + ")");
                                temp1 = Integer.parseInt(tokens[k].replaceAll("[^\\d]", ""));
                            } else {
                                pos = search(tokens[k], symb_table, total_symb);
                                if (fill_addr && pos != -1) {
            ...
            for (int i = 0; i < total_symb; i++)
                bw.write(symb_table[i].name + "\t" + symb_table[i].addr + "\n");

            bw.write("\n*****POOL TABLE***\n");
            bw.write("POOL\tTOTAL LITERALS\n");
            for (int i = 0; i < pooltab_cnt; i++)
                bw.write(pooltab[i].first + "\t" + pooltab[i].total_literals + "\n");
            ...
            for (int i = 0; i < total_ltr; i++) {
                if (literal_table[i].addr == 0)
                    literal_table[i].addr = loc++;
                bw.write(i + "\t" + literal_table[i].name + "\t" + literal_table[i].addr + "\n");
            }
            bw.close();
        } catch (Exception e) {
        ...
        try (BufferedReader br2 = new BufferedReader(new FileReader("Intermediate.txt"))) {
            System.out.println("\n*****Intermediate.txt****\n");
